...
### Added
- Integrated `hyshot-editor` library for annotation and opening existing images.
- Named external-tool configuration through `hyshot external NAME`.
- Configurable OCR and Translate Shell examples using selected-region PNG input.
- Per-workflow freeze settings for area screenshots, built-in annotation, and
  external tools.

### Changed
- Unified Hyshot configuration and desktop workflow; no independent Annotator
  process or configuration is required.
- Release assets use the Hyshot name and include the GPL license and notices.
