    ...
    pub fn draw(&mut self, app: &mut Application) {
        ...
        while let Some(command) = self.window_context.commands.pop_front() {
            match command {
                ...
                Command::CopyImage(receiver) => match receiver.recv() {
                    Ok(rgba_image) => {
                        if let Ok(png_bytes) = crate::clipboard::to_png_bytes(&rgba_image) {
                    ...
                    Err(err) => {
                        log::warn!("Failed to receive screenshot: {:?}", err);
                    }
                },
                Command::SaveImage(receiver, exit_after_save) => match receiver.recv() {
                    Ok(image) => match crate::clipboard::to_png_bytes(&image) {
                        Ok(png_bytes) => match crate::image_save::save_png(&png_bytes) {
                            Ok(path) => {
                                crate::notify::saved(path.display());
                                if exit_after_save {
                        ...
                        Err(err) => log::warn!("Failed to encode screenshot: {err}"),
                    },
                    Err(err) => log::warn!("Failed to receive screenshot: {err:?}"),
                },
