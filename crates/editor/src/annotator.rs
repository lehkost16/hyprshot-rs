...
use crate::platform::dpi::{LogicalSize, Pixel};
...
impl AnnotatorState {
    ...
    pub fn submit_annotation(&mut self, annotation: Annotation) {
        ...
        let config = self.session.settings();
    ...
    /// Update session preferences; the host persists them after editing ends.
    pub fn save_current_config(&mut self) {
        let saved_config = self.session.settings();
